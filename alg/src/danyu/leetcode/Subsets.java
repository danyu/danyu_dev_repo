...
*Solution:
*For each index, two possibilities:present or absent.
*So there are 2^N subsets.
*Each subset can be represent by a integer, which can 
*be decoded into a series of indices, which can guide assembling of
*this subset.
*
**/
...
public class Subsets {
	...
	public ArrayList<ArrayList<Integer>> subsets(int[] S) {
	    ...
	    for(int i=0;i<NUM_OF_SETS;i++){
	    	...
	    	//check bit by bit
	    	while( setInt> 0){
	    		if((setInt & 1)>0) newSet.add(S[index]);
	    		setInt=setInt>>1;
	    		index++;
	    	}
	    	result.add(newSet);
	...
	public static void merge(Comparable[] bk, Comparable[] a, int lo, int mid,int hi) {
		...
		for (int k = lo; k <= hi; k++) {
			...
			else if (bk[i].compareTo(bk[j])<0) a[k] = bk[i++];
			else           			     a[k] = bk[j++];
		}
	}
	...
	public static void main(String[] args) {
		...
		ss.printResult(ss.subsets(new int[]{6,1,2,5,4}));
	}
